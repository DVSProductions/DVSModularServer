...
namespace System {
	...
	public static class ServerFrameWork {
		...
		public static void QUWI<T>(string name, Action<T> action, T passThis) => ThreadPool.QueueUserWorkItem((e) => { Thread.CurrentThread.Name = name; action((T)e); }, passThis);
		/// <summary>
		...
		private static string RecursiveToString(object o, ref HashSet<object> knownOBJS, int maxdepth = Int32.MaxValue) {
			...
			if (t.IsArray || t is IEnumerable<object>) {
				ret.Append("ArrayElements = [");
				foreach (var e in o as IEnumerable<object>) {
					try {
