...
public class EncryptedServerConfig {
	...
	/// </summary>
	public string CryptoFileName { get; set; } = string.Empty;
	/// <summary>
	...
	/// </summary>
	public EncryptedServerConfig() => EncryptedFiles = [];
	/// <summary>
	...
	/// <returns></returns>
	public static EncryptedServerConfig Load(TextReader reader) =>
		new XmlSerializer(typeof(EncryptedServerConfig)).Deserialize(reader) is not EncryptedServerConfig config
			? throw new InvalidDataException("Failed to deserialize EncryptedServerConfig.")
			: config;
}
