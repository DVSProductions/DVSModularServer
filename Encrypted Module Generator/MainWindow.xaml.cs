...
public partial class MainWindow : Window {
	...
	private readonly EncryptedServerConfig cfg;
	private EncryptedServerConfig.EncryptionType encryptionType;
	private readonly List<string> localFiles;
	private string? serverFilePath, cryptoPath;
	private readonly bool[] status = [false, false, false, false, false, false];
	...
	private void ComboBox_SelectionChanged(object sender, SelectionChangedEventArgs e) {
		if(btEnc == null || sender is not ComboBox cb)
			return;
		...
		if(dlg.ShowDialog() == true) {
			...
			try {
				using(var cat = new AggregateCatalog()) {
					...
					cat.Catalogs.Add(new DirectoryCatalog(path, Path.GetFileName(dlg.FileName)));
					var loader = new CompositionContainer(cat);
					loader.ComposeParts(this);
				}
				cryptoPath = dlg.FileName;
				status[3] = true;
				bt.Content = Path.GetFileName(dlg.FileName);
			}
			catch(Exception ex) {
	...
	private void Button_Click_3(object sender, RoutedEventArgs e) {
		...
		try {
			...
			if(encryptionType == EncryptedServerConfig.EncryptionType.CUSTOM) {
				if(string.IsNullOrEmpty(cryptoPath)) {
					MessageBox.Show("No Crypto DLL Specified", "Error");
					return;
				}
