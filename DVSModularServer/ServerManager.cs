...
#pragma warning restore CS8618 // Non-nullable field must contain a non-null value when exiting constructor. Consider adding the 'required' modifier or declaring as nullable.
	...
	public void Stop() {
		...
		if(Servers.Count != 0 && lazyServerList != null) {
			...
			while(enders.Count != 0) {
				if(enders.Peek().ThreadState != ThreadState.Running)
					enders.Pop().Join();
				else
					Task.Delay(100);
			}
	...
	private bool ImportServer(Lazy<IServer, IServerMeta> s) {
		try {
			...
			if((pairs == null || pairs.Count == 0) && store.serv is not IServer2)
...
#pragma warning restore CA2000
	...
	private void LoadModules() {
		...
		foreach(var a in lst) {
			...
			catalog.Catalogs.Add(toDispose[^1]);
		}
...
#pragma warning restore CA2000
	...
	private static string BuildErrorPage(HttpStatusCode statusCode, string msg) =>
	...
	private static string ShowErrorPage(HttpListenerResponse response, ServerFrameWork.errorPage? p, HttpStatusCode statusCode, string msg) {
	...
	/// <param name="msg">Error Message</param>
	private static string ShowErrorPage(HttpListenerResponse response, HttpStatusCode statusCode, string msg) => ShowErrorPage(response, null, statusCode, msg);
	/// <summary>
