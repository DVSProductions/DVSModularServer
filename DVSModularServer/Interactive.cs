...
namespace DVSModularServer {
	sealed partial class Interactive : IInteractive {
		bool printed = false, queryModeActive = false, InCommand = false;
		...
		string hiddenText = null, typedText = "";
		static HashSet<char> valid;
		readonly List<string> commandHistory;
		...
		public string GetTypedText() { printed = true; return queryModeActive ? getPromptText?.Invoke() : prefix + typedText; }
		/// <summary>
		...
		static bool IsValidChar(char c) => valid.Contains(c);
		/// <summary>
		/// Splits the command into command and Arguments
		/// </summary>
		/// <param name="s">prompt input</param>
		static public Tuple<string, List<string>> SplitCommand(string s) {
			string command = null;
			...
			add(s.Length - start);
			return new Tuple<string, List<string>>(command, ret);
		}
		/// <summary>
		...
		private void CommandInputMode(ConsoleKeyInfo k, Action onEnter, Action<ConsoleKeyInfo> SpecialCaseHandler = null) {
			...
			switch(k.Key) {
				...
				case ConsoleKey.UpArrow:
					if((historyIndex <= 0 && hiddenText != null) || commandHistory.Count == 0)
						...
						if(hiddenText == null)
							...
							historyIndex--;
						while(historyIndex >= commandHistory.Count) historyIndex--;
						HideTypedText();
				...
				case ConsoleKey.RightArrow:
					if(typedIndex >= typedText.Length) Beep();
					else {
						typedIndex++;
						Console.CursorLeft = CursorX;
					}
				...
				default:
					if(!IsValidChar(ch))
						...
						if(hiddenText != null) hiddenText = null;
						if(typedIndex != typedText.Length) HideTypedText();
		...
		/// <param name="k">pressed key</param>
		delegate void keydownAction(string prompt, ref string typedText, ref int typedIndex, ConsoleKeyInfo k);
		/// <summary>
		...
		private static void DefaultKeydown(string prompt, ref string typedText, ref int typedIndex, ConsoleKeyInfo k) {
			if(IsValidChar(k.KeyChar)) {
				if(typedIndex != typedText.Length) HideStr(prompt, typedText);
		...
		private void TextModeWrapper(string prompt, Action<string> onEnter, keydownAction keydown = null, Action<ConsoleKeyInfo> SpecialCaseHandler = null) {
			...
			var typedIndex = 0;
			if(keydown == null) keydown = DefaultKeydown;
		...
		private void PasswordModeWrapper(string prompt, Action<string> onEnter, Action<char> storeChar, char disguiseChar = '*', keydownAction keydown = null, Action<ConsoleKeyInfo> SpecialCaseHandler = null) {
			if(keydown == null) keydown = (string p, ref string tT, ref int tI, ConsoleKeyInfo k) => {
				if(tI != tT.Length) HideStr(p, tT);
		...
		private void CommandModeExecute() {
			...
			if(o.Item1 == null)
				...
				try {
					...
					typedText = "";
					typedIndex = 0;
					comm.Execute(o.Item2);
					if(!printed) Console.Write(prefix);
					InCommand = false;
					while(queryModeActive) Thread.Sleep(100);
				}
				catch(Exception ex) {
		...
		Action<ConsoleKeyInfo> onQueryModeKeyPress;
		//private object k;
		/// <summary>
		...
		public bool PromptYN(string msg) => Prompt(msg, new[] { 'y', 'n' }) == 0;
		/// <summary>
